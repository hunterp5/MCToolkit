...
class AppMenuMixin:
    def init_menubar(self):
        ...
        if calc is not None and load_config().disable_custom_calc:
            ...
            calc.setToolTip("Calculator disabled by MCTOOLKIT_DISABLE_CUSTOM_CALC.")
    ...
    def _ensure_protein_viewer(self, *, show: bool = True):
        ...
        needs_canvas = bool(
            dlg is not None and (created or not getattr(dlg, "_canvas_bootstrapped", False))
        )
        if dlg is not None and needs_canvas and not getattr(dlg, "_canvas_bootstrapped", False):
            begin = getattr(dlg, "begin_canvas_load", None)
            if callable(begin):
                from ..strings import (
            ...
            if has_content:
                ...
                if callable(setter):
                    setter(payload)
        if dlg is not None and needs_canvas:
            prepare = getattr(getattr(dlg, "viewer", None), "prepare_to_show", None)
            if callable(prepare):
                prepare()
        if show and dlg is not None:
            ...
            if needs_canvas and callable(flush):
                flush()
        if needs_canvas and dlg is not None:
