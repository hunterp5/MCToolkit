...
class ChemistryWorkspaceWindow(
    ...
    def __init__(self):
        ...
        cfg = load_config()
        # Cap via MCTOOLKIT_MAX_THREADPOOL (1–64); otherwise scale gently with CPU count.
        ...
        self._tautomer_browser_dialog = None
        self._tautomer_browse_records = []
        self._protomer_browser_dialog = None
        self._protomer_browse_records = []
    ...
    def changeEvent(self, event) -> None:  # noqa: N802 — Qt API name
        ...
        if event is None or event.type() != QEvent.WindowStateChange:
            return
        if self.isMinimized():
            return
        mgr = getattr(self, "_workspace_layout", None)
        refresh = getattr(mgr, "refresh_splitter_handles", None)
        if callable(refresh):
            refresh()
