...
class FilterBounds:
    ...
    def calculate_global_bounds(self, *, on_complete=None) -> None:
        ...
        self._app._bounds_on_complete = on_complete
        self._app._bounds_chunk_gen = int(getattr(self._app, "_bounds_chunk_gen", 0)) + 1
        self._app._bounds_chunk_active = False
    ...
    def _finish_bounds_calculation(self) -> None:
        ...
        cb = getattr(self._app, "_bounds_on_complete", None)
        self._app._bounds_on_complete = None
        if cb is not None:
            QTimer.singleShot(0, cb)
            return
        if hasattr(self._app, "status_label") and (
            not getattr(self._app, "_render2d_batch_active", False)
        ):
            cur = self._app.status_label.text()
    ...
    def _set_bounds_prep_progress(self, message: str) -> None:
        ...
        if getattr(self._app, "_ingest_prep_before_reveal", False) and hasattr(
            self._app, "_loading_detail"
        ):
            self._app._loading_detail.setText(message)
        if hasattr(self._app, "status_label") and (
            not getattr(self._app, "_render2d_batch_active", False)
        ):
            self._app.status_label.setText(message.replace("\n", " — "))

    def _apply_global_bounds_sync(self) -> None:
        self._app.global_bounds = self._app._table_model.numeric_bounds_by_column()
    ...
    def _refresh_bounds_on_filter_cards(self) -> None:
        ...
        for f in self._app.filters:
            if isinstance(f, FilterCard):
                f.update_prop_list(list(self._app.global_bounds.keys()))
        ...
        refresh_plot_axes = getattr(self._app, "_refresh_active_plot_axis_columns", None)
        if callable(refresh_plot_axes):
            refresh_plot_axes()

    def _begin_chunked_global_bounds(self) -> None:
        headers = self._app._table_model.list_bounds_data_headers()
        if not headers:
            self._apply_global_bounds_sync()
            self._finish_bounds_calculation()
            return
        self._app._bounds_chunk_active = True
        self._app._bounds_chunk_headers = headers
        self._app._bounds_chunk_col_i = 0
        self._app._bounds_chunk_row_i = 0
        self._app._bounds_chunk_acc: dict[str, dict] = {}
        gen = int(self._app._bounds_chunk_gen)
    ...
    def _bounds_chunk_step(self, gen: int) -> None:
        if gen != int(getattr(self._app, "_bounds_chunk_gen", -1)):
            return
        if not getattr(self._app, "_bounds_chunk_active", False):
            return
        cfg = load_config()
        row_budget = int(cfg.bounds_chunk_rows)
        deadline = time.monotonic() + max(0.005, int(cfg.ingest_gui_time_budget_ms) / 1000.0)
        headers = self._app._bounds_chunk_headers
        ci = int(self._app._bounds_chunk_col_i)
        n_rows = self._app._table_model.rowCount()
        processed = 0
        while ci < len(headers) and processed < row_budget and (time.monotonic() < deadline):
            header = headers[ci]
            start = int(self._app._bounds_chunk_row_i)
            remain = row_budget - processed
            end = min(start + remain, n_rows)
            acc = self._app._bounds_chunk_acc.get(header)
            self._app._bounds_chunk_acc[header] = self._app._table_model.merge_numeric_bounds_chunk(
                header, start, end, acc
            )
            processed += end - start
            if end >= n_rows:
                if (
                    header in self._app._bounds_chunk_acc
                    and self._app._bounds_chunk_acc[header] is None
                ):
                    self._app._bounds_chunk_acc.pop(header, None)
                ci += 1
                self._app._bounds_chunk_row_i = 0
            else:
                self._app._bounds_chunk_row_i = end
        self._app._bounds_chunk_col_i = ci
        if ci < len(headers):
            self._set_bounds_prep_progress(f"Preparing filters…\n({ci}/{len(headers)} columns)")
            QTimer.singleShot(0, lambda g=gen: self._bounds_chunk_step(g))
            return
        self._app._bounds_chunk_active = False
        self._app._table_model.install_numeric_bounds_cache(self._app._bounds_chunk_acc)
        self._app.global_bounds = dict(self._app._bounds_chunk_acc)
        self._refresh_bounds_on_filter_cards()
        self._finish_bounds_calculation()
    ...
    def schedule_calculate_global_bounds(self, *, delay_ms: int | None = None) -> None:
        ...
        timer = getattr(self._app, "_bounds_recalc_timer", None)
        if timer is None:
            return
        cfg = load_config()
