...
class Molecule3DViewerWidget(Mol3DConfMixin, Mol3DChromeMixin, QWidget):
        ...
        *,
        ...
        try:
            ...
            msg = (
                ...
                "• Start mctoolkit with `python -m mctoolkit` so QtWebEngine loads before the GUI initializes.\n\n"
    ...
    def _on_export_to_table(self) -> None:
        ...
        try:
            web.page().runJavaScript(
                "window.mctoolkitConfState ? mctoolkitConfState() : null",
                _finish,
            )
        except Exception:
            self._btn_export_table.setEnabled(True)
    ...
    def _refit_standalone_viewer(self, web, *, zoom: bool = False) -> None:
        ...
        if web is None:
            return
        js = (
            "if (window.mctoolkitRefit) { mctoolkitRefit({zoom: "
