...
from .dialogs.tautomer import TautomerGeneratorDialog
...
class PredictJobState(Protocol):
    ...
    _tautomer_browse_records: Any
    _tautomer_browser_dialog: Any
    _protomer_browse_records: Any
    _protomer_browser_dialog: Any
...
class PredictTools:
    ...
    def _on_named_browser_dialog_destroyed(self, attr: str) -> None:
        from .qt_widget_utils import qobject_is_deleted
        ...
        try:
            ...
            return
        current = getattr(self._app, attr, None)
        if sender is not None and current is not None and (current is not sender):
            return
        setattr(self._app, attr, None)

    def _on_tautomer_browser_dialog_destroyed(self, *_args) -> None:
        self._on_named_browser_dialog_destroyed("_tautomer_browser_dialog")

    def _on_protomer_browser_dialog_destroyed(self, *_args) -> None:
        self._on_named_browser_dialog_destroyed("_protomer_browser_dialog")

    def _discard_stale_form_browser_dialog(self, attr: str) -> None:
    ...
    def _open_form_browser(
        ...
        *,
        ...
        if self._host_unavailable() or not groups:
            return
        setattr(self._app, records_attr, list(groups))
        self._discard_stale_form_browser_dialog(dialog_attr)
        ...
        for w in self._app.iter_docked_plot_widgets():
            if isinstance(w, widget_cls):
                ...
                w.set_groups(groups)
                w.raise_()
                self._app.status_label.setText(f"{status_tool}: focused in workspace pane.")
                return

        def _factory():
            dlg = dialog_cls(self._app)
        ...
        reuse_or_show_modeless_singleton(
            self._app, dialog_attr, _factory, on_reused_visible=_on_reused
        )

    def open_tautomer_browser(self, rows) -> None:
        """Show enumerated tautomers in a 2D pose-style browser."""
        from .tautomer_browser import (
            TautomerBrowserDialog,
            TautomerBrowserWidget,
            groups_from_tautomer_rows,
        )

        groups = groups_from_tautomer_rows(rows)
        self._open_form_browser(
            groups,
            records_attr="_tautomer_browse_records",
            dialog_attr="_tautomer_browser_dialog",
            widget_cls=TautomerBrowserWidget,
            dialog_cls=TautomerBrowserDialog,
            status_tool="Tautomers",
        )

    def open_protomer_browser(self, rows) -> None:
        """Show enumerated protomers in a 2D pose-style browser."""
        from .protomer_browser import (
            ProtomerBrowserDialog,
            ProtomerBrowserWidget,
            groups_from_protomer_rows,
        )

        groups = groups_from_protomer_rows(rows)
        self._open_form_browser(
            groups,
            records_attr="_protomer_browse_records",
            dialog_attr="_protomer_browser_dialog",
            widget_cls=ProtomerBrowserWidget,
            dialog_cls=ProtomerBrowserDialog,
            status_tool="Generate Protomers",
        )
    ...
    def open_tautomer_generator(self) -> None:
        if not ensure_table_ready_for_tool(self._app, "Tautomers"):
            return

        dlg = TautomerGeneratorDialog(self._app)
        self._app._prepare_tool_dialog(dlg)
        dlg.setAttribute(Qt.WA_DeleteOnClose, True)
        dlg.show()
        dlg.raise_()
