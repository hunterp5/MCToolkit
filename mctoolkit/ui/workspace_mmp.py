...
"""Matched molecular pair (MMP) analysis on WorkspaceTools."""
...
class MmpTools:
    ...
    def open_mmp_dialog(self) -> None:
        ...
        show_activity_tool_dialog(self._app, d, on_accepted=self._on_mmp_dialog_accepted)
    ...
    def on_mmp_finished(self, pairs, activity_column: str) -> None:
        ...
        if rows:
            self._app.on_calc_finished(rows, headers, finish_progress=False)

        self._app._mmp_last_pairs = pairs
        ...
        self._app.status_label.setText(f"MMP: {len(pairs)} pair(s).")

    def on_mmp_failed(self, message: str) -> None:
        report_analysis_failure(self._app, TOOL_MMP, message, fallback="MMP analysis failed.")

    def open_mmp_transform_ledger_for_last_run(self) -> None:
        """Re-open the Transform Ledger for the most recent MMP analysis."""
        pairs = list(getattr(self._app, "_mmp_last_pairs", None) or [])
    ...
    def _open_mmp_ledger(self, pairs, *, activity_column: str) -> None:
        ...
        def _factory():
            dlg = MmpTransformLedgerDialog(self._app, pairs, activity_column=activity_column)
            dlg.setModal(False)
    ...
    def _open_mmp_browser(self, pairs, *, activity_column: str) -> None:
        ...
        def _factory():
            dlg = MmpBrowserDialog(self._app, pairs, activity_column=activity_column)
            dlg.setModal(False)
            dlg.setWindowModality(Qt.NonModal)
            return dlg
