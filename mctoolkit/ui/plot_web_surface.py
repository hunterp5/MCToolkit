...
# Qt HWND resize does not fire window.resize inside Chromium; poke the shell so it can
# quiet-debounce into a single Plotly.Plots.resize (settle policy).
...
class _PlotWebHostFilter(QObject):
    ...
    The shell quiet-debounces that poke into a single ``Plots.resize`` after size settles.
    """
