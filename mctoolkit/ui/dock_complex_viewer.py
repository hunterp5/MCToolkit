    ...
    function mctoolkitInitView() {
      try {
        ...
        function modelIndices() {
          var recIdx = window.mctoolkitRecB64 ? 0 : -1;
          var ligIdx = window.mctoolkitLigB64 ? (recIdx >= 0 ? 1 : 0) : -1;
        ...
        function applyAllStyles(v, styles) {
          ...
          var st = styles || window.mctoolkitRenderStyles || {};
          clearLabels(v);
        ...
        window.mctoolkitRecB64 = "";
        window.mctoolkitRecFmt = "pdb";
        window.mctoolkitLigB64 = "";
        window.mctoolkitLigFmt = "sdf";
        window.mctoolkitRenderStyles = {receptor: "cartoon", ligand: "ballstick", pocket: "stick"};
        installResetStructureMenu();
        window.mctoolkitResizeKeepView = function () { keepViewResize(window.mctoolkitViewer); };
        window.mctoolkitApplyRenderStyles = function (styles) {
          if (!window.mctoolkitViewer) return;
          if (styles) window.mctoolkitRenderStyles = styles;
          applyAllStyles(window.mctoolkitViewer, window.mctoolkitRenderStyles);
          keepViewResize(window.mctoolkitViewer);
        };
        window.mctoolkitApplyPocketView = function () {
          if (!window.mctoolkitViewer) return;
          window.mctoolkitRenderStyles = {
            receptor: "cartoon",
            ligand: "ballstick",
            pocket: "ballstick",
            labels: true
          ...
          var idx = modelIndices();
          applyAllStyles(v, window.mctoolkitRenderStyles);
          fitToLigand(v, idx.ligIdx, 0.85);
        };
        window.mctoolkitSetDockComplexPayload = function (payload) {
          if (!window.mctoolkitViewer || !payload) return;
          var v = window.mctoolkitViewer;
          window.mctoolkitRecB64 = payload.rec || "";
          window.mctoolkitRecFmt = payload.recFmt || "pdb";
          window.mctoolkitLigB64 = payload.lig || "";
          window.mctoolkitLigFmt = payload.ligFmt || "sdf";
          if (payload.styles) window.mctoolkitRenderStyles = payload.styles;
          var refit = !!payload.refit;
          v.clear();
          try { v.removeAllSurfaces(); } catch (eClr) {}
          var recIdx = -1;
          var ligIdx = -1;
          if (window.mctoolkitRecB64) {
            v.addModel(atob(window.mctoolkitRecB64), window.mctoolkitRecFmt);
            recIdx = 0;
          }
          if (window.mctoolkitLigB64) {
            v.addModel(atob(window.mctoolkitLigB64), window.mctoolkitLigFmt);
            ligIdx = recIdx >= 0 ? 1 : 0;
          }
          applyAllStyles(v, window.mctoolkitRenderStyles);
          if (refit) {
...
{_viewer_dock_complex_init_script()}
  <script src="{script_src}" onload="mctoolkitInitView()"></script>
</body>
</html>"""
...
class DockComplexEmbedView(QWidget):
    ...
    def apply_pocket_view(self) -> None:
        ...
        try:
            self._web.page().runJavaScript(
                "if (window.mctoolkitApplyPocketView) window.mctoolkitApplyPocketView();"
            )
        except Exception:
    ...
    def set_render_style(self, component: str, style: str) -> None:
        ...
        if self._web is None or not self._web_ready:
            return
        js = (
            "if (window.mctoolkitApplyRenderStyles) "
            f"window.mctoolkitApplyRenderStyles({json.dumps(payload_styles)});"
        )
        try:
    ...
    def _run_payload(self, payload: dict) -> None:
        if self._web is None or not self._web_ready:
            ...
            return
        js = (
            "if (window.mctoolkitSetDockComplexPayload) "
            f"window.mctoolkitSetDockComplexPayload({json.dumps(payload)});"
        )
        try:
            self._web.page().runJavaScript(js)
        except Exception:
