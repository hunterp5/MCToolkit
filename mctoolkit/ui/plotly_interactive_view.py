...
class PlotlyInteractiveView(QWidget):
    ...
    def _prepare_figure_for_shell(self, fig: go.Figure, oids: list[int]) -> None:
        ...
        if "mctoolkit_selection_traces" not in meta:
            indices: list[int] = []
        ...
        meta["mctoolkit_hover_persist"] = bool(self._hover_persist)
        fig.update_layout(meta=meta, clickmode="event+select", dragmode="lasso")

        custom = [[int(oid)] for oid in oids]
        sel_traces = meta.get("mctoolkit_selection_traces") or [0]
    ...
    def _sync_hover_persist_visual(self) -> None:
        ...
        self.web.page().runJavaScript(
            f"window.mctoolkitSetHoverPersist && mctoolkitSetHoverPersist({json.dumps(persist)});"
        )
        if not persist:
            self.web.page().runJavaScript(
                "window.mctoolkitClearHoverPin && mctoolkitClearHoverPin();"
            )
            return
        ...
        self.web.page().runJavaScript(
            f"window.mctoolkitPinHoverPoints && mctoolkitPinHoverPoints({json.dumps(js_idxs)});"
        )
