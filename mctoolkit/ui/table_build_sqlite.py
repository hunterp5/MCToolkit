...
class TableBuildSqlite:
    def _rebuild_sqlite_store_from_model(self) -> None:
        ...
        if store is None:
            return
        self._app._sqlite_rebuild_in_progress = True
        data_headers = [
            h
            for h in self._app.headers[2:]
            if h and not self._app._table_model.is_pixmap_data_column(h)
        ]
        entries = self._app._table_model.export_rows_for_sqlite(data_headers)
        ...
        try:
            with scope("sqlite.rebuild"):
                store.rebuild(self._app.headers, entries)
    ...
    def _schedule_sqlite_rebuild(self) -> None:
        ...
        store = getattr(self._app, "_sqlite_store", None)
        if store is None or self._app._sqlite_rebuild_in_progress:
            return
        sigs = getattr(self._app, "_sqlite_rebuild_signals", None)
        pool = getattr(self._app, "threadpool", None)
        if sigs is None or pool is None:
            self._rebuild_sqlite_store_from_model()
            return
        self._app._sqlite_rebuild_gen = int(getattr(self._app, "_sqlite_rebuild_gen", 0)) + 1
        gen = self._app._sqlite_rebuild_gen
        self._app._sqlite_rebuild_in_progress = True
        data_headers = [
            h
            for h in self._app.headers[2:]
        ...
        fd, db_path = tempfile.mkstemp(prefix="MCTOOLKIT_sqlite_", suffix=".sqlite3")
        try:
            os.close(fd)
        except OSError:
            pass
        self._app._sqlite_rebuild_pending_path = Path(db_path)
        n_rows = self._app._table_model.rowCount()
        from .background_jobs import register_background_job

        job_id = f"sqlite-rebuild-{gen}"
        self._app._sqlite_rebuild_bg_job_id = job_id
        ...
        self._app.status_label.setText(f"Indexing table… (0/{n_rows:,} rows)")
        QTimer.singleShot(0, self._sqlite_export_chunk_step)

    def _sqlite_export_chunk_step(self) -> None:
        ctx = getattr(self._app, "_sqlite_export_ctx", None)
        if not ctx or ctx.get("gen") != getattr(self._app, "_sqlite_rebuild_gen", -1):
            self._close_sqlite_rebuild_writer(ctx)
            self._app._sqlite_export_ctx = None
            return
        if not self._app._sqlite_rebuild_in_progress:
            self._close_sqlite_rebuild_writer(ctx)
            self._app._sqlite_export_ctx = None
            return
        ...
        slice_rows = self._app._table_model.export_rows_for_sqlite_slice(data_headers, row_idx, end)
        ...
        db_path = ctx["db_path"]
        sigs = ctx["signals"]
        headers = list(ctx.get("headers") or self._app.headers)
        self._close_sqlite_rebuild_writer(ctx)
        self._app._sqlite_export_ctx = None
        pool = getattr(self._app, "threadpool", None)
        if pool is None:
    ...
    def _on_sqlite_rebuild_finished(self, job_gen: int, db_path: str) -> None:
        ...
        try:
            ...
            self._app._sqlite_store = new_store
            if old is not None:
                try:
                    old.close()
                except Exception:
        ...
        if getattr(self._app, "_sqlite_rebuild_stale", False):
            self._app._sqlite_rebuild_stale = False
            self._app._sqlite_store_dirty = True
            self._schedule_sqlite_rebuild()
            return
        n_rows = self._app._table_model.rowCount()
        if getattr(self._app, "_sqlite_rebuild_pending_filters", False):
            self._app._sqlite_rebuild_pending_filters = False
    ...
    def _on_sqlite_rebuild_failed(self, job_gen: int, msg: str) -> None:
        ...
        self._app.status_label.setText(
            "Table indexing failed — filters may be slow until data changes."
        )
