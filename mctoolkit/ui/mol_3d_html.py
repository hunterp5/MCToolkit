...
_RESET_STRUCTURE_JS = r"""
        function captureHomeView(v) {
          ...
          try {
            var home = v.getView();
            if (home && home.length) window.mctoolkitHomeView = home.slice();
          } catch (eH) {}
        }
        function applyHomeView(v) {
          ...
          var home = window.mctoolkitHomeView;
        ...
        function installResetStructureMenu() {
          ...
          window.mctoolkitResetStructure = function () { hideResetMenu(); applyHomeView(); };
        }
"""
...
def _viewer_init_script_fragment(mol_b64: str, *, flat: bool) -> str:
    ...
    function mctoolkitInitView() {
      try {
        ...
        window.mctoolkitRefit = function (opts) {
          if (!window.mctoolkitViewer) return;
          if (opts && opts.zoom) fitAndCapture(window.mctoolkitViewer);
          else {
            try { window.mctoolkitViewer.resize(); } catch (e0) {}
...
def _viewer_embed_init_script_fragment(mol_b64: str = "", *, flat: bool = False) -> str:
    """Minimal 3Dmol init for sketcher side panel: no atom pick UI; live ``mctoolkitSetMolB64``."""
    flat_js = "true" if flat else "false"
    tmpl = r"""  <script>
    function mctoolkitInitView() {
      try {
        __RESET_JS__
        window.mctoolkitFlat = __FLAT__;
        const opts = window.mctoolkitFlat
          ? { backgroundColor: "white", orthographic: true }
          : { backgroundColor: "white" };
        const viewer = $3Dmol.createViewer("v", opts);
        window.mctoolkitViewer = viewer;
        installResetStructureMenu();
        window.mctoolkitRefit = function () { fitAndCapture(window.mctoolkitViewer); };
        window.mctoolkitSetMolB64 = function (b64, flat) {
          if (typeof flat !== "undefined") window.mctoolkitFlat = !!flat;
          if (!window.mctoolkitViewer) return;
          var v = window.mctoolkitViewer;
          var isFlat = !!window.mctoolkitFlat;
          v.clear();
          if (b64) {
        ...
        window.mctoolkitSetMolB64("__MOLB64__");
        window.addEventListener("resize", function () {
          if (window.mctoolkitRefit) window.mctoolkitRefit();
        });
      } catch (e) {
        document.body.innerHTML = "<pre style='padding:12px;font-family:monospace'>3Dmol error: " + e + "</pre>";
      }
...
{reset_menu}
{init_html}
{extra}  <script src="{script_src}" onload="mctoolkitInitView()"></script>
</body>
</html>"""
    ...
    *,
    ...
    function mctoolkitInitView() {
      try {
        ...
        window.mctoolkitLoadConf = function (i) { loadConf(i); };
        window.mctoolkitShowSuperpose = function (idxs, opts) { showSuperpose(idxs, opts); };
        window.mctoolkitSetConfLegend = setConfLegend;
        window.mctoolkitConfState = function () {
