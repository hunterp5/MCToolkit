...
class WorkspaceTools:
    ...
    def __init__(self, app: WorkspaceToolsHost) -> None:
        ...
        self.cluster = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_cluster", "ClusterTools")
        )
        self.dimension_reduction = _LazyCollaborator(
            app,
            lambda: _load("mctoolkit.ui.workspace_dimred", "DimensionReductionTools"),
        )
        self.medchem_space = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_medchem", "MedChemSpaceTools")
        )
        self.qsar = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_qsar", "QsarTools")
        )
        self.mpo = _LazyCollaborator(app, lambda: _load("mctoolkit.ui.workspace_mpo", "MpoTools"))
        self.mmp = _LazyCollaborator(app, lambda: _load("mctoolkit.ui.workspace_mmp", "MmpTools"))
        self.mmp_neighborhood = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_mmp_neighborhood", "MmpNeighborhoodTools")
        )
        self.activity_cliff = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_activity_cliff", "ActivityCliffTools")
        )
        self.sali = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_sali", "SaliTools")
        )
        self.structure_prep = _LazyCollaborator(app, _load_structure_prep)
        self.viewers = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_viewers", "ViewerOpenersTools")
        )
        self.external = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_external", "ExternalRecordsTools")
        )
        self.table_calc = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_table_calc", "TableCalcTools")
        )
        self.fragment = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_fragment", "FragmentTools")
        )
        self.reaction = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_reaction", "ReactionTools")
        )
        self.descriptors = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_descriptors", "DescriptorsTools")
        )
        self.conformers = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_conformers", "ConformersTools")
        )
        self.sql_load = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_sql_load", "SqlLoadTools")
        )
        self.dock = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_dock", "DockTools")
        )
        self.predict = _LazyCollaborator(
            app, lambda: _load("mctoolkit.ui.workspace_predict", "PredictTools")
        )
...
def _load_structure_prep():
    ProtonateTools = _load("mctoolkit.ui.workspace_protonate", "ProtonateTools")
    FastPrepareTools = _load("mctoolkit.ui.workspace_fast_prepare", "FastPrepareTools")
    StructureEditTools = _load("mctoolkit.ui.workspace_structure_edit", "StructureEditTools")
    StructureWritebackTools = _load(
        "mctoolkit.ui.workspace_structure_writeback", "StructureWritebackTools"
    )
