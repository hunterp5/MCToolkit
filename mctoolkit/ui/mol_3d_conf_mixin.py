...
"""Conformer table, overlay scope, and 3Dmol.js navigation — file-split of ``Molecule3DViewerWidget``."""
...
class Mol3DConfMixin:
    ...
    def _sync_conf_legend(self) -> None:
        ...
        self._run_viewer_js(f"if (window.mctoolkitSetConfLegend) mctoolkitSetConfLegend({arg});")
    ...
    def _refresh_3d_from_scope(self) -> None:
        ...
        if not vis:
            self._conf_superposed = False
            self._set_superpose_checked(False)
            self._run_viewer_js("if (window.mctoolkitShowSuperpose) mctoolkitShowSuperpose([]);")
            self._sync_conf_legend()
            return
    ...
    def _show_superpose(self) -> None:
        ...
        if len(vis) < 2:
            if vis:
                ...
                self._set_superpose_checked(False)
                self._run_viewer_js(
                    "if (window.mctoolkitShowSuperpose) mctoolkitShowSuperpose([]);"
                )
                self._sync_conf_legend()
            return
        ...
        self._run_viewer_js(
            "if (window.mctoolkitShowSuperpose) "
            f"mctoolkitShowSuperpose({payload}, {{zoom: {zoom_js}, colors: {colors_js}}});"
        )
        self._sync_conf_legend()
    ...
    def _show_conformer(self, conf_idx: int, *, preserve_selection: bool = True) -> None:
        ...
        if table is not None:
            ...
            try:
                ...
                table.blockSignals(False)
        self._run_viewer_js(f"if (window.mctoolkitLoadConf) mctoolkitLoadConf({idx});")
        self._sync_conf_legend()
