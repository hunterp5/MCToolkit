...
class ProteinViewerIoMixin:
    ...
    def open_mmgbsa_dialog(self) -> None:
        """Open the MM-GBSA dialog for a Manager structure or file."""
        dlg = self._mmgbsa_dialog
        if dlg is not None and qobject_is_deleted(dlg):
            self._mmgbsa_dialog = None
            dlg = None
        if dlg is None:
            from .dialogs.protein_mmgbsa import ProteinMMGBSADialog

            dlg = ProteinMMGBSADialog(self)
            dlg.scored.connect(self._on_mmgbsa_scored)
            dlg.destroyed.connect(self._on_mmgbsa_dialog_destroyed)
            self._mmgbsa_dialog = dlg
        dlg.prefill_from_viewer()
        dlg.show()
        dlg.raise_()
        dlg.activateWindow()

    def _on_mmgbsa_dialog_destroyed(self) -> None:
        self._mmgbsa_dialog = None

    def _on_mmgbsa_scored(self, result) -> None:
    ...
    def open_md_dialog(self) -> None:
        """Open the implicit MD dialog for a Manager structure or file."""
        dlg = self._md_dialog
        if dlg is not None and qobject_is_deleted(dlg):
            self._md_dialog = None
            dlg = None
        if dlg is None:
            from .dialogs.protein_md import ProteinMDDialog

            dlg = ProteinMDDialog(self)
            dlg.finished_md.connect(self._on_md_finished)
            dlg.destroyed.connect(self._on_md_dialog_destroyed)
            self._md_dialog = dlg
        dlg.prefill_from_viewer()
        dlg.show()
        dlg.raise_()
    ...
    def _on_md_dialog_destroyed(self) -> None:
        self._md_dialog = None
    ...
    def _workspace_dock_host(self):
        """Main window that owns Gnina, PDBQT prepare, and the pose browser."""
        parent = self.parent()
        if parent is not None and callable(getattr(parent, "open_gnina_dock", None)):
            return parent
        return None

    def _forward_workspace_dock(self, method_name: str) -> None:
        host = self._workspace_dock_host()
        fn = getattr(host, method_name, None) if host is not None else None
        if callable(fn):
            fn()

    def open_dock_prepare(self) -> None:
        self._forward_workspace_dock("open_dock_prepare")

    def open_dock_prepare_pdb(self) -> None:
        self._forward_workspace_dock("open_dock_prepare_pdb")

    def open_gnina_dock(self) -> None:
        self._forward_workspace_dock("open_gnina_dock")

    def open_dock_results_viewer(self) -> None:
        self._forward_workspace_dock("open_dock_results_viewer")

    def _sync_dock_viewer_action(self) -> None:
        ...
        host = self._workspace_dock_host()
        if host is not None:
            main_act = getattr(host, "_act_dock_viewer", None)
            if main_act is not None:
                try:
                    enabled = bool(main_act.isEnabled())
                except RuntimeError:
                    enabled = False
            if not enabled:
                snap = getattr(host, "_last_dock_results", None) or {}
                enabled = bool(isinstance(snap, dict) and snap.get("mols"))
        try:
