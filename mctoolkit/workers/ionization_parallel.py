...
from ..platform_support.env import env_get, env_pop, env_set
...
def unipka_cuda_available() -> bool:
    ...
    from mctoolkit.ionization.unipka_ensembles import pka_gpu_forced_off, torch_is_cuda_build
...
def _set_unipka_mmff_thread_env(proc_workers: int) -> str | None:
    ...
    prev = env_get("MCTOOLKIT_UNIPKA_MMFF_THREADS")
    if prev:
        return prev
    if proc_workers > 1:
        env_set("MCTOOLKIT_UNIPKA_MMFF_THREADS", "1")
    return None
...
def _restore_unipka_mmff_thread_env(prev: str | None, wrote: bool) -> None:
    if not wrote:
        return
    if prev is None:
        env_pop("MCTOOLKIT_UNIPKA_MMFF_THREADS")
    else:
        env_set("MCTOOLKIT_UNIPKA_MMFF_THREADS", prev)
...
def predict_microstates_for_sketch(
    ...
    *,
    ...
    from mctoolkit.ionization.unipka_ensembles import microstates_for_mol
...
def build_microstates_cache_by_key(
    ...
    *,
    ...
    from mctoolkit.ionization.microstate_cache import lookup as cache_lookup
    from mctoolkit.ionization.microstate_cache import store_many as cache_store_many
