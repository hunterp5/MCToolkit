...
def _is_explicit_solvent(name: str) -> bool:
    raw = (name or "").strip().lower().replace(" ", "")
    return raw in {_SOLVENT_TIP3P, "explicit", "pme", "tip3pbox", "waterbox"}


def _normalize_openmm_platform(name: str) -> str:
    raw = (name or _OPENMM_PLATFORM_AUTO).strip()
    env = (env_get("MCTOOLKIT_OPENMM_PLATFORM") or "").strip()
...
def _amber_topology_positions(prmtop_path: Path, inpcrd_path: Path):
    """Load Amber topology and coordinates without building a System."""
...
def _amber_explicit_system(
    ...
    *,
    ...
    """PME + optional MonteCarloBarostat from a solvated AmberTools prmtop."""
    ...
    try:
        ...
        raise RuntimeError(
            f"OpenMM could not build a PME system from the solvated AmberTools prmtop. {exc}"
        ) from exc
    if float(pressure_bar) > 0:
        system.addForce(
            MonteCarloBarostat(
    ...
    label = f"TIP3P PME NPT {float(pressure_bar):.2f} bar"
    if float(pressure_bar) <= 0:
        label = "TIP3P PME NVT"
    return system, prmtop.topology, inpcrd.positions, label
...
def add_position_restraints(
    ...
    *,
    ...
    original_ca_keys: set[ResidueKey] | None = None,
    ligand_keys: set[ResidueKey] | None = None,
) -> int:
    """Harmonic position restraints. Returns the number of restrained atoms.

    The force uses a global ``k`` so MD can zero or reduce it after equilibration
    with ``context.setParameter("k", …)`` without rebuilding the system.
    """
    from openmm import CustomExternalForce, unit

    from .protein_prepare_qc import restrain_atom

    restraint_set = _normalize_restraint_set(scheme)
    ...
    original_keys = original_ca_keys or _ca_residue_keys(topology)
    n_restrained = 0
    for atom in topology.atoms():
...
def _restrained_minimize_pdb(
    ...
    *,
    ...
    add_position_restraints(
        ...
        pdb.topology,
        pdb.positions,
        scheme=restraint_set,
        k_kcal_per_ang2=float(k_kcal_per_ang2),
        original_ca_keys=restrained_ca_keys,
        ligand_keys=ligand_keys,
    )
