...
def hide_owned_windows_console() -> bool:
    """Hide a console this process allocated (``mctoolkit.exe``), not an inherited terminal.

    Legacy console ``[project.scripts]`` wrappers open a terminal; ``[project.gui-scripts]``
    avoids that. Double-clicking an old console launcher still opens a terminal that
    terminal and then the workspace — two windows. Leave the console alone when the
    user launched from PowerShell, cmd, or an IDE terminal (more than one process
    is attached).
    """
