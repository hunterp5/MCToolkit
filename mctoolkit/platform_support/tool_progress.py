...
_progress_job_id: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "mctoolkit_tool_progress_job_id",
    default=None,
)
...
class ToolProgressState:
    ...
    Concurrent jobs write named slots so Processes can show each row's own text.
    """
...
def format_overlay_progress_text(message: str, done: int, total: int) -> str:
    """Loading-overlay text: message plus count and percent when *total* is known."""
    if total < 0:
        return str(message or "")
    dv = min(max(int(done), 0), int(total))
    tot = int(total)
...
def throttled_progress_should_emit(
    done: int,
    total: int,
    throttle: list,
    *,
    ...
    """True when a count should be shown. *throttle* is ``[last_done, last_monotonic]``."""
    d = int(done)
    tot = int(total)
    now = time.monotonic()
    last_d = int(throttle[0]) if throttle else 0
    last_t = float(throttle[1]) if len(throttle) > 1 else 0.0
    if tot <= 0:
        if last_t <= 0.0 or (now - last_t) >= interval_s:
            throttle[0] = d
            throttle[1] = now
            return True
        return False
    step = max(1, tot // 20)
    if d <= 0 or d >= tot or d - last_d >= step or last_t <= 0.0 or (now - last_t) >= interval_s:
        throttle[0] = d
        throttle[1] = now
        return True
    return False
