...
def tautomer_dialog_footer_html() -> str:
    """Rich text appended under the tautomer generator hint."""
    return (
        "<small><b>Method</b>: RDKit <code>TautomerEnumerator</code> "
        "(MolStandardize / MolVS transforms and tautomer scoring). "
        "Lower-scoring quinoid and enol outliers are dropped. "
        "This is connectivity tautomerism, not ionization — use "
        "<b>Generate Protomers</b> for charge states.</small>"
    )


def mmgbsa_dialog_footer_html() -> str:
    """Rich text for Protein Viewer → Simulate → MM-GBSA."""
    return (
        "<small><b>Method</b>: 1-trajectory <b>MM-GBSA</b> — "
        '<a href="https://doi.org/10.1021/ja00124a002">Srinivasan et al., JACS 1998</a>; '
        '<a href="https://doi.org/10.1021/ar000033j">Kollman et al., Acc. Chem. Res. 2000</a>. '
        "OpenMM GBn2/OBC2 on a GAFF2 holo topology (AmberTools). "
        "ΔG = E<sub>complex</sub> − E<sub>receptor</sub> − E<sub>ligand</sub> "
        "(kcal/mol). No −TΔS. Ranking score, not experimental ΔG.</small>"
    )


def md_dialog_footer_html() -> str:
    """Rich text for Protein Viewer → Simulate → Molecular Dynamics."""
    return (
        "<small><b>Method</b>: OpenMM Langevin MD — implicit GBn2/OBC2 or explicit TIP3P "
        "(PME, Monte Carlo barostat). Optional 1-trajectory MM-GBSA on snapshots "
        "(solute only if the run was solvated). "
