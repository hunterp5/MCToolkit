...
4. Later Protonate / LogD jobs reuse the session cache for the same structures. **Save Session** writes those ensembles into the ``.mct`` file so Open / Duplicate does not need to re-run Uni-pKa.
