...
- **3Dmol.js** — `mctoolkit/ui/static/3Dmol-min.js` (offline 3D viewer)
...
mctoolkit/resources/bin/linux/ # Linux
mctoolkit/resources/bin/mac/   # macOS
```

Override the search directory with environment variable `MCTOOLKIT_BUNDLE_DIR`.
...
`mctoolkit/resources/models/biotransformer/` — see `models/biotransformer/README.md`. Download:

```bash
python scripts/bootstrap_biotransformer.py
```

Requires Java on PATH plus the official JAR (`BioTransformer3.0_20230525.jar` or `biotransformer-3.0.0.jar`) with sibling `btkb/` or `database/` and `supportfiles/`. Override the JAR with `MCTOOLKIT_BIOTRANSFORMER_JAR`, or use **Browse JAR…** in Predict Metabolites.
