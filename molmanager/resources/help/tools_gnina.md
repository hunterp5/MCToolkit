...
- **Pharmacophore** - optional MolManager pharmacophore JSON from **Protein Viewer → Pharmacophore** (Save, or **Send to Gnina**). Gnina has no native pharmacophore flag; MolManager writes an AutoDock map (`--user_grid`) of Gaussian wells at each enabled feature (Exclusion sites are repulsive). **λ** is `--user_grid_lambda` (default 1). Feature types are used for coloring and RDKit “From ligand”; the map itself is occupancy-based. The map covers the numeric search box, or the feature bounding box plus Autobox padding.
