...
- **Sequence** — editable sequence of every chain, including amino acids, **lowercase letters for residues missing from the coordinates** (SEQRES / `_pdbx_poly_seq_scheme` gaps), ligands/cofactors (`+`), metals (`*`), and waters (`~`); selected observed characters highlight in 3D. This is the 3D-linked chain editor, not a multiple-sequence alignment. For FASTA/MAFFT alignment use **Protein → Sequence** on the main window.
- **Prepare…** — docking-ready cleanup for **Smina**: PDBFixer repair, pdb2pqr/PROPKA protein protonation at a chosen pH (ligand stays in for holo titration), Uni-pKa ligand protomer at that pH with optional pocket Coulomb reweight (SMILES/SDF, or mmCIF `_chem_comp_bond`), optional Manager-selected or ligand-near waters, then OpenMM restrained minimization (on by default). After chemistry, Prepare writes an **apo receptor PDBQT** (Meeko), a **crystal ligand PDB/SDF**, and a **search box** from a ligand in the **loaded structure** or from a **separate ligand file** (default 4 Å padding; shown in **View → Docking Box**). Use the file option when the ligand is not in the structure being prepared. **Open Smina…** fills Tools → Dock → Smina with those files and the numeric box. **Keep ligand in prepared mmCIF** is on by default so the overlay stays holo; the PDBQT is still apo. **Restrained minimization** is on by default. You choose **protein force field** (ff14SB or ff99SB-ILDN), **ligand force field** (protein only, or GAFF2/GAFF via AmberTools — WSL on Windows), **solvation** (GBn2 GBSA, OBC2, or vacuum), salt, and whether to restrain **backbone**, **Cα**, or **backbone + ligand**. Protein-only minimization holds the ligand out of OpenMM and restores it afterward if **Keep ligand in prepared mmCIF** is on. GAFF/GAFF2 keeps the ligand in the OpenMM system (AM1-BCC charges when AmberTools `sqm` succeeds). Highest-occupancy altlocs are kept; SEQRES gaps are modeled with PDBFixer (not only PDBFixer's residue-number alignment, which misses tagged N-termini and kinase inserts). SEQRES loops next to the ligand can still be skipped. Output remarks include pocket HIS/ASP/GLU names and Cα/pocket RMSD vs the start of minimization. Default output is **mmCIF** (PDB is optional) so ligand bond orders are written as `_chem_comp_bond`. **mmCIF inputs stay mmCIF for the whole Prepare run**. pdb2pqr still uses a PDB/PQR scratch file internally (PQR is the format that carries per-atom charges); that is converted back immediately so HID/HIE/HIP, ASH, GLH names, hydrogens, and ligand bonds survive in the mmCIF. Per-atom pdb2pqr charges are not stored in standard mmCIF/`ATOM` records — keep the scratch `.pqr` if you need them for APBS. PDBQT is the docking export, not the archival protonation file. The result is **overlaid** on the original instead of replacing it.
- **Minimize…** — restrained OpenMM minimization of the **loaded protein–ligand complex** without PDBFixer/pdb2pqr. Default is **GAFF2** (AmberTools; WSL on Windows) with backbone + ligand heavy-atom restraints and GBn2 GBSA so the ligand stays in the OpenMM system. **Protein only** holds the ligand out and restores it afterward. Supply ligand **SMILES** or an **SDF/MOL2** unless the file is mmCIF with `_chem_comp_bond`. The minimized structure is **overlaid** on the current file. Use this after **Prepare…** (or on an already protonated complex) when you want pose relaxation without rewriting Smina files.
...
- **View → Docking Box** — show the Smina search box written by **Prepare…** (ligand bounding box plus padding).
