...
This is **not** Data → SOM (self-organizing map). Predictions use FAME3R models hosted on NERDD; they are free for non-commercial research. Highlighted atom indices match the preprocessed structure on the map. Failed molecules leave N/A text and no image. **Cancel** keeps completed molecules: they are written to the table and opened in the browser. If you cancel a **Phase 1 and 2** run after Phase 1 finishes, Phase 1 sites are still written. Dock the browser with **Add to Main Window** if you want it beside the table.
