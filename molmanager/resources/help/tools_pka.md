...
For amines this is the aqueous pKa of the **conjugate acid** (BH+ ⇌ B + H+), which is the number tabulated in references (dimethylamine ≈ 10.7, not the much higher pKa of the free base as an acid). Uni-pKa’s free-energy head is trained on mean-centered Dwar targets; MolManager adds that mean so the table matches the official aqueous scale. Older **UniPka** sample columns written before that shift sit near ~4–6 for aliphatic amines and should be ignored — re-run **Predict pKa** / **Protonate** to refresh the shared **pKa** column.
