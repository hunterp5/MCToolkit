...
The in-memory log covers the current MolManager process. It is not saved with the session. A rotating file log is still written under the user log directory when file logging is enabled.
...
- **Open log file** - open the rotating `molmanager.log` in the default editor when file logging is on.
