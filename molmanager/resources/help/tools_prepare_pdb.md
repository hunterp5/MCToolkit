...
Automated fixes can mis-handle cofactors - inspect the site. This does not flex side chains for induced fit. Always visually check the binding pocket after prep. Protein Viewer **Tools → Prepare → Fast Prepare…** runs a fuller pipeline (pdb2pqr/PROPKA protein protonation, Uni-pKa ligand protomer at the same pH with optional pocket Coulomb reweight, and OpenMM GBn2 GBSA restrained minimization — protein-only AMBER by default, or GAFF/GAFF2 via AmberTools/WSL for the complex) on the structure already in the 3D canvas. **Prepare → PDBFixer…** is the in-viewer equivalent of this tool's repair/clean step without adding hydrogens.
