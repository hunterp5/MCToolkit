...
from mctoolkit.platform_support.config import clamp_substructure_async_rows, load_config
...
def test_load_config_sql_caps_respect_monkeypatch(monkeypatch):
    monkeypatch.setenv("MCTOOLKIT_SQL_MAX_ROWS_HARD", "5000")
    monkeypatch.setenv("MCTOOLKIT_SQL_PRECOUNT_WARN", "2000")
    cfg = load_config()
...
def test_protomer_process_workers_env(monkeypatch):
    monkeypatch.setenv("MCTOOLKIT_PROTOmer_PROCESSES", "3")
    assert load_config().protomer_process_workers == 3


def test_pka_process_workers_env(monkeypatch):
    monkeypatch.setenv("MCTOOLKIT_PKA_PROCESS_WORKERS", "2")
    assert load_config().pka_process_workers == 2


def test_disable_custom_calc_env(monkeypatch):
    monkeypatch.setenv("MCTOOLKIT_DISABLE_CUSTOM_CALC", "1")
    assert load_config().disable_custom_calc is True


def test_legacy_custom_calc_eval_env_is_ignored(monkeypatch, caplog):
    monkeypatch.setenv("MCTOOLKIT_CUSTOM_CALC_LEGACY_EVAL", "1")
    import logging

    with caplog.at_level(logging.WARNING, logger="mctoolkit.platform_support.config"):
        cfg = load_config()
...
def test_perf_and_sqlite_page_size_env(monkeypatch):
    monkeypatch.setenv("MCTOOLKIT_PERF_METRICS", "true")
    monkeypatch.setenv("MCTOOLKIT_PERF_LOG_EVERY", "5")
    monkeypatch.setenv("MCTOOLKIT_SQLITE_BACKEND_PAGE_SIZE", "6000")
    cfg = load_config()
...
def test_structure_render_limits_env(monkeypatch):
    monkeypatch.setenv("MCTOOLKIT_AUTO_RENDER_2D_MAX_ROWS", "12000")
    monkeypatch.setenv("MCTOOLKIT_STRUCTURE_RENDER_LAZY_MIN_ROWS", "6000")
    monkeypatch.setenv("MCTOOLKIT_STRUCTURE_RENDER_PIXMAP_LRU", "128")
    cfg = load_config()
...
def test_tool_progress_poll_ms_env(monkeypatch):
    monkeypatch.setenv("MCTOOLKIT_TOOL_PROGRESS_POLL_MS", "350")
    assert load_config().tool_progress_poll_ms == 350


def test_descriptor_fp_process_pool_env(monkeypatch):
    monkeypatch.setenv("MCTOOLKIT_DESCRIPTOR_FP_PROCESS_POOL_MIN_ROWS", "128")
    monkeypatch.setenv("MCTOOLKIT_DESCRIPTOR_PROCESS_POOL_BATCH_SIZE", "16")
    cfg = load_config()
