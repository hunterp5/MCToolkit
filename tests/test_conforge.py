...
from molmanager.conforge import (
    ConforgeParams,
    confgen_cli_command,
...
def test_ensure_conforge_ready_reports_missing(monkeypatch):
    monkeypatch.setattr("molmanager.conforge.python_conforge_available", lambda: False)
    monkeypatch.setattr("molmanager.conforge.resolve_confgen_executable", lambda _p="": None)
...
def test_run_conforge_generation_merges_sdf(monkeypatch):
    ...
    sdf = block + "$$$$\n" + block + "$$$$\n"
    monkeypatch.setattr("molmanager.conforge.ensure_conforge_ready", lambda _p="": None)
    monkeypatch.setattr("molmanager.conforge._run_conforge", lambda _sdf, _p: sdf)
