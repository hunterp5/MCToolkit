...
from mctoolkit.conformers.openbabel_confab import SystematicConfParams
from mctoolkit.workers.openbabel_confab_worker import SystematicConformerWorker
...
def test_systematic_worker_emits_packed_confs(qapp, monkeypatch):  # noqa: ARG001
    ...
    monkeypatch.setattr(
        "mctoolkit.workers.openbabel_confab_worker.ensure_openbabel_confab_ready",
        lambda _p="": None,
    )
    monkeypatch.setattr(
        "mctoolkit.workers.openbabel_confab_worker.run_systematic_conformer_generation",
        _fake_run,
    )
    sig = WorkerSignals()
    captured: list = []
...
def test_systematic_worker_missing_openbabel(qapp, monkeypatch):  # noqa: ARG001
    ...
    assert cell.get("err") == "openbabel_unavailable"
